...
public class ProjApplication {
	...
	public CommandLineRunner demo(PersonService personService, PersonAdder personAdder){
		return (args -> {
			personService.createUser(new Person("John", "Doe","cosik@asd.pl", "password", Permissions.ADMIN));
			personService.createUser(new Person("John", "Smith","abc@asd.pl","pwd",Permissions.USER));
			// fetch all customers
			Logger log = Logger.getLogger("logger");
			log.info("Customers found with findAll():");
			log.info("-------------------------------");
			personService.getAllUsers().forEach(customer -> log.info(customer.toString()));
